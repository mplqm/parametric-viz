...
N_FRAMES          = 60
MIN_RESIDUES      = 20
MIN_CLUSTER_SIZE  = 2   # skip singleton↔singleton transitions (viewer unusability)
...
# Only align structures that appear in clusters
...
# ── 4. Generate transitions and write directly to transitions/ ────────────────
n_pairs = len(clusters) * (len(clusters) - 1) // 2
print(f"\nGenerating up to {n_pairs} pairwise transitions…")

# Clear and recreate the transitions/ folder
...
print(f"\nWrote {written} transitions  (skipped {skipped} with < {MIN_RESIDUES} shared Cα)")
print(f"Index: transitions/index.json  ({idx_kb:.0f} KB)")
print(f"Total files in transitions/: {len(os.listdir(OUT_DIR))}")
...
if len(index_entries) > 20:
    print(f"  … {len(index_entries) - 20} more transitions")
